...
def simple_line_plot(ts):
    """
    plotly line plot of monthly toronto indicators
    """
    return multi_plot(ts)


def indexed_line_plot(ts):
    """
    plotly line plot of monthly toronto indicators
    index to Jan 2020 = 100
    """
    indexed = ts.divide(ts.iloc[36]).multiply(100).round(0)

    return multi_plot(indexed)


def yoy_line_plot(ts):
    """
    plotly line plot of monthly toronto indicators
    YoY
    """
    yoy = ts.pct_change(periods=12, fill_method=None).multiply(100).round(0)

    return multi_plot(yoy)